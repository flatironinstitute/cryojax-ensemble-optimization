...
from cryojax_ensemble_optimization.ensemble_optimization._pose_search.base import global_SO3_hier_search
...
@pytest.mark.parametrize("num_points, grid_size, n_quaternions, sigma", [
    (4, 20, 7, 0.2),
])
def test_global_SO3_hier_search(num_points, grid_size, n_quaternions, sigma):
    ...
    values_gt = jnp.sum(jax.vmap(lambda c: gaussian_basis(grid, c, sigma))(points_gt), axis=0)
    ...
    def lossfn(quaternions):
        ...
        values_est = jnp.sum(evaluate_gaussians_on_grid(rotated_points, grid, sigma=sigma), axis=1)
        return jnp.linalg.norm((values_est - values_gt).reshape(n_quaternions, -1), axis=1)

    quaternions = Rsp.random(n_quaternions).as_quat()
    _ = lossfn(quaternions)  # Just to check it runs

    best_quats, _ = global_SO3_hier_search(lossfn, base_grid=1, n_rounds=5, N_candidates=40)

    assert np.allclose(best_quats, quaternion_gt, atol=0.1) or np.allclose(best_quats, -quaternion_gt, atol=0.1), \
