...
def simulator_(
    ...
    # assert pixel_size < 2.0 * resolution, "Pixel size should be smaller than 2.0 * resolution due to the Nyquist limit."

    gauss_var = resolution ** 2#struct_info[0, :] * resolution**2
    gauss_amp = 1.0 #struct_info[1, :] / jnp.sqrt(gauss_var * 2.0 * jnp.pi) ** 3
...
def simulate_stack(
    ...
    if not models.shape[0] == len(images_per_model):
        logging.info("Number of models and number of images per model do not match. Exiting.")
        raise ValueError("Number of models and number of images per model do not match.")
    ...
    image_stack = load_starfile(root_path, starfile_fname, batch_size=batch_size, res=config["resolution"])
